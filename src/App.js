...
function App() {
  ...
  const ethicsScores = {
    'amazon': { score: 30, reason: 'Tax avoidance, labor issues', color: '#ff4444' },
    'world of books': { score: 85, reason: 'Circular economy, B Corp', color: '#44ff44' },
    'better world books': { score: 90, reason: 'Funds literacy programs', color: '#44ff44' },
    ...
    'back market': { score: 85, reason: 'Refurbished electronics', color: '#44ff44' },
    'charity shops': { score: 95, reason: 'Funds good causes', color: '#44ff44' },
  ...
  const parseWishlistText = (text) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      else if (line.length > 10 && !line.match(/^(Title|Price|Quantity|Has|Comments)/) && !line.match(/^\d+\s+of\s+\d+/)) {
  ...
  const categorizeItem = (text) => {
    ...
    const categoryRules = [
      { category: 'books', keywords: ['book', 'novel', 'paperback', 'hardcover', 'kindle', 'author', 'read', 'story', 'tales', 'guide'] },
      { category: 'electronics', keywords: ['electronic', 'phone', 'cable', 'charger', 'usb', 'computer', 'laptop', 'tablet', 'camera', 'headphone'] },
      { category: 'toys', keywords: ['toy', 'game', 'puzzle', 'play', 'child', 'kid', 'lego', 'board game'] },
      { category: 'garden', keywords: ['garden', 'plant', 'seed', 'outdoor', 'flower', 'lawn', 'soil'] },
      { category: 'home', keywords: ['kitchen', 'home', 'decor', 'furniture', 'lamp', 'cushion', 'table'] },
      { category: 'health', keywords: ['beauty', 'health', 'care', 'cosmetic', 'skincare', 'vitamin', 'wellness'] },
      { category: 'fashion', keywords: ['clothing', 'shirt', 'dress', 'shoes', 'jacket', 'fashion', 'wear'] }
    ];
    ...
    if (file && file.type === 'application/pdf') {
      ...
      try {
        ...
        if (items.length > 0) {
          ...
          alert("No items could be extracted. Please make sure this is an Amazon wishlist PDF.");
          setLoading(false);
        }
      } catch (error) {
  ...
  const analyzeWishlistItems = (items) => {
    ...
    Object.keys(categories).forEach(category => {
      ...
      analysisResults[category] = {
        retailers: retailers.map(name => ({
          name,
          ...(ethicsScores[name.toLowerCase()] || { score: 50, reason: 'No data', color: '#aaaaaa' })
  ...
  const getRetailersForCategory = (category) => {
    const retailerMap = {
      'books': ['World of Books', 'Better World Books', 'ThriftBooks', 'Hive', 'Waterstones', 'Amazon'],
  ...
  const handleURLSubmit = () => {
    if (wishlistURL) {
      setLoading(true);
      setTimeout(() => {
        alert("URL processing requires the wishlist to be public. For best results, please use the PDF upload option.");
        setLoading(false);
      }, 1500);
    }
  };
  ...
  const quickSearch = () => {
    ...
    else if (searchLower.includes('electronic') || searchLower.includes('phone')) category = 'electronics';
    else if (searchLower.includes('toy') || searchLower.includes('game')) category = 'toys';
    
    const retailers = getRetailersForCategory(category);
    const scoredRetailers = retailers.map(name => ({
      name,
      ...(ethicsScores[name.toLowerCase()] || { score: 50, reason: 'No data', color: '#aaaaaa' })
    })).sort((a, b) => b.score - a.score);
  ...
  return (
    <div className="App">
        ...
        <div className="content">
          <div className="upload-section">
            ...
            <div className="upload-box">
              ...
              <p className="help-text">Print your Amazon wishlist to PDF and upload it here</p>
              <p className="help-text" style={{fontSize: '12px', marginTop: '5px'}}>
          ...
          <div className="results-section">
            {results.type === 'wishlist' && (
              <div className="summary">
                ...
                {results.extractedItems && results.extractedItems.length > 0 && (
                  <div className="extracted-preview">
                    ...
                    {results.itemCount > 5 && (
                      <p className="more-items">...and {results.itemCount - 5} more items</p>
                    )}
                  </div>
                )}
              </div>
            ...
            {Object.entries(results.data).map(([category, data]) => (
              <div key={category} className="category-results">
                <h3>
                  {category === 'Search Results' ? '' : category.charAt(0).toUpperCase() + category.slice(1)}
                ...
                {data.sampleItems && data.sampleItems.length > 0 && (
                  <div className="sample-items">
                    <p>Including: {data.sampleItems.map(item => item.title).join(', ')}</p>
                  </div>
                )}
            ...
            <button className="new-search-button" onClick={() => {
              setActiveTab('upload');
              setResults(null);
              setExtractedItems([]);
              setUploadedFileName('');
      ...
      <footer>
        <p>Data sources: B Corporation Directory • WikiRate • Public Information</p>
